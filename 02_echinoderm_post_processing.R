...
tibble(lat_range = conflict_ranges) %>%
  ...
  summarise(
    ...
    n_under_0001            = sum(lat_range < 0.0001),
    pct_under_0001          = round(mean(lat_range < 0.0001) * 100, 1),
    n_under_001             = sum(lat_range < 0.001),
    pct_under_001           = round(mean(lat_range < 0.001)  * 100, 1),
    n_under_05              = sum(lat_range < 0.5),
    pct_under_05            = round(mean(lat_range < 0.5)  * 100, 1),
    n_over_1deg             = sum(lat_range > 1),
...
coord_conflict_flag <- conflict_ranges > 0.5 & !is.na(conflict_ranges)
...
# Manual override — CAT_J.5636
# Only two sources report coordinates (AM_direct, OBIS), ~715 km apart; no
# GBIF value is available to break the tie and iDigBio (listed in
# sources_all) has no populated coordinates for this record, so the
# automated logic above defaulted to averaging AM_direct and OBIS —
# producing a point in open water consistent with neither source.
# verbatimLocality ("just north of North Direction Island, Great Barrier
# Reef") matches AM_direct's coordinates (~14.7°S, near Lizard Island); the
# OBIS coordinates (~21.2°S, near Mackay) are inconsistent with this
# locality. AM_direct is retained as authoritative.
echino_wide <- echino_wide %>%
  mutate(
    best_latitude  = if_else(record_key == "CAT_J.5636", -14.73, best_latitude),
    best_longitude = if_else(record_key == "CAT_J.5636", 145.5,  best_longitude)
  )

cat("Records with meaningful coord conflict (>0.5°):",
...
# Manual overrides — excluded from the automated majority/GBIF tally below
# since they were resolved by individual review, not by either automated rule
manual_coord_overrides <- c("CAT_J.5636")

conflicted_rows <- which(coord_conflict_flag & !(echino_wide$record_key %in% manual_coord_overrides))
...
cat("  Resolved by individual manual review:", length(manual_coord_overrides), "\n")
# =============================================================================
...
# New Caledonia country correction
# MTQ_CIDARIS (Museum of Tropical Queensland) and AM_direct/iDigBio institutional
# exports default country to "Australia" (the depositing institution's own location)
# even when the specimen's own verbatimLocality clearly documents a New Caledonia
# collection locality. These records are retained (biogeographic continuity with
# the Coral Sea study region - see geographic scope), but their country field is
# corrected here so exported metadata matches the specimen's actual origin.
nc_locality_pattern <- "new\\s*caledonia|noum[eé]a"

echino_wide %>%
  filter(str_detect(str_to_lower(coalesce(verbatimLocality, "")), nc_locality_pattern)) %>%
  summarise(
    n = n(),
    lat_min = round(min(best_latitude, na.rm = TRUE), 3),
    lat_max = round(max(best_latitude, na.rm = TRUE), 3),
...
# New Caledonia (6)      → reassign from Australia to New Caledonia (verbatimLocality
#                          names Nouméa/New Caledonia explicitly; institutional CMS
#                          exports default country to depositing museum's own location).
#                          Retained in dataset per biogeographic continuity with the
#                          Coral Sea study region.
# australia (20,456)     → standardise to Australia
# Commonwealth of Australia (31) → standardise to Australia
# NA (4,610)             → assign Australia (all within bbox, confirmed above)
# =============================================================================

echino_wide <- echino_wide %>%
  mutate(
    country = case_when(
      str_detect(str_to_lower(coalesce(verbatimLocality, "")), nc_locality_pattern) ~ "New Caledonia",
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# ADDITION: Per-record license resolution ("most restrictive wins")
# -----------------------------------------------------------------------------
# The 'license' field is a substantial share of records (~24% by original
...
# not respect.
#
# Reads from echino_long_license_fields.csv - a companion export produced by
# the integration script (01_echinoderm_dataset_integration.R, right after
# echino_long.csv is written) containing ONLY record_key, source, license,
# dcterms:license and rights. This avoids loading the full echino_long.csv
# (340MB / 413 columns) just to read 5 of them, and sidesteps readr's
# type-inference on the 'rights' column, which was previously misread on the
# full file (mixed types across chunks) and could tank the join.
#
# We use this narrower Layer A file (one row per contributing source per
...
echino_long_for_license <- read_csv(
  "echino_long_license_fields.csv",
  show_col_types = FALSE,
  col_types = cols(
    record_key        = col_character(),
    source             = col_character(),
    license            = col_character(),
    `dcterms:license`  = col_character(),
...
license_by_record <- echino_long_for_license %>%
  mutate(
    license_any = coalesce(.data[["license"]], .data[["dcterms:license"]]),
...
eml_xml <- c(
  ...
  '    <coverage>',
  '      <geographicCoverage>',
  '        <geographicDescription>Northeast Australia and the adjacent Coral Sea, spanning shelf, slope and abyssal habitats from the Torres Strait to southeast Queensland, including the Gulf of Carpentaria and adjacent Coral Sea waters. This definition extends beyond strict jurisdictional limits in some cases but ensures ecological coherence and enables consistent assessment of taxonomic completeness across the full shelf-to-abyssal gradient. Records from Papua New Guinea, New Caledonia, and adjacent Coral Sea waters were retained given their biogeographic continuity with the study region.</geographicDescription>',  '        <boundingCoordinates>',
  '          <westBoundingCoordinate>136</westBoundingCoordinate>',
  '          <eastBoundingCoordinate>167</eastBoundingCoordinate>',
  '          <northBoundingCoordinate>-9</northBoundingCoordinate>',
  ...
  '    </coverage>',
  '    <maintenance>',
  '      <description><para>Static snapshot corresponding to source download dates listed in the associated Data paper.</para></description>',
  ...
  '      <methodStep>',
  '        <description>',
  '          <para>Occurrence records were integrated from 13 source datasets, harmonised to Darwin Core, deduplicated using a hierarchical occurrenceID / catalogNumber / composite-key strategy, and cross-source conflicts in coordinates, collection year, depth, basis of record and taxonomic name were resolved through a scripted R workflow. All scientific names were validated against the World Register of Marine Species (WoRMS). Per-record data-quality flags are provided in the MeasurementOrFact extension and in dynamicProperties. Full methods are described in the associated Data paper.</para>',
  '        </description>',
  '      </methodStep>',
  '    </methods>',
  '    <project>',
