...
cat("\nRemaining records still missing coordinates:",
...
echino_wide <- echino_wide %>%
  ...
  mutate(
    coord_land_qc_flag = coalesce(coord_land_qc_flag, "not_on_land"),
...
#
# REMOVE (1,651): all other material samples
#   — AODN environmental eDNA (1,020)
#   — OBIS DNA sequence reads (487)
#   — BCUH/CUKES tissue subsamples (26)
#   — BOLD/EMBL DNA barcoding (70)
#   — iBOL ECLI tissue samples (38)
#   — iBOL OGL/Lomonosov MSU (99)
#   — KX GBIF duplicates of QM G22502 (3)
#   — Other OBIS no-voucher (1)
#   — CAT BOLD Australian records (62) ← included in BOLD/EMBL above
...
cat("Material samples removed:", n_before - nrow(echino_wide), "\n")
cat("Records remaining:", nrow(echino_wide), "\n\n")
...
#
# Final result: 19,038 of 42,905 records have depth (44.4%), up from
# 18,327 (42.7%) before the two recovery steps.
# =============================================================================
...
cat("Records gained via reef flat / intertidal inference:",
    sum(!is.na(echino_wide$best_min_depth) | !is.na(echino_wide$best_max_depth)) - n_before_5g, "\n\n")
...
#
# Final result: 42,905 records (4 Disaster genus records reclassified to
# Section 1 geographic scope exclusions; 42 non-echinoderm contaminants
# removed here in Section 6.0), 100% WoRMS-validated (every record has a
...
name_resolution <- echino_wide %>%
  ...
  pivot_longer(cols = all_of(clean_cols), names_to = "source_col", values_to = "queried_name") %>%
  ...
  group_by(record_key) %>%
  summarise(
    accepted_name = final_name[which.min(status_rank)][1],
    accepted_name_rank = worms_rank[which.min(status_rank)][1],
...
#   - "Amphioplus (Amphichilus) ochroleuca": now correctly resolves to
#     "Amphioplus (Amphichilus) ochroleucus" (grammatical gender agreement).
#   - "Amphioplus (Amphioplus) intermedius": now correctly resolves to
#     "Amphioplus (Amphichilus) intermedius" (confirmed via direct AphiaID
#     lookup: AphiaID 242815 status = "unaccepted > superseded combination",
#     accepted name AphiaID 244924 = Amphioplus (Amphichilus) intermedius).
# -----------------------------------------------------------------------------
...
verified_valid_names <- tribble(
  ...
  # --- ADDITIONS: alternative representation -> preferred accepted form ---
  "Amphiodia (amphispina) microplax",          "Amphiodia (Amphispina) microplax",
  # --- ADDITIONS: lowercase subgenus formatting fix ---
...
echino_wide <- echino_wide %>%
  mutate(is_extinct_genus = accepted_name_final == "Hemiaster")
...
# Higher ranks: where accepted_name_final is a known class/order/family name,
# the single-word rule above assigns "Genus" which is wrong.
# Use worms_rank_master to correct these where available.
echino_wide <- echino_wide %>%
  mutate(
    taxonomic_resolution_level = case_when(
      worms_rank_master %in% c("Class","Subclass","Superclass") ~ "Class",
      worms_rank_master %in% c("Order","Suborder","Superorder") ~ "Order",
      worms_rank_master %in% c("Family","Subfamily","Superfamily") ~ "Family",
      worms_rank_master == "Phylum" ~ "Phylum",
      TRUE ~ taxonomic_resolution_level
    )
  )
...
source_priority <- c(
  ...
  "MTQ_CIDARIS", "CIDARIS_QMT", "GBIF_QM", "GBIF_echinodermata", "OZCAM",
  "iDigBio", "OBIS"
)
...
echino_wide <- echino_wide %>%
  ...
  mutate(
    inferred_class = if_else(taxonomic_resolution_level == "Class", accepted_name_final, taxon_class),
...
# -----------------------------------------------------------------------------
# # FIX APPLIED: counts updated to reflect two reorganisations:
#   1. 4 Disaster genus records moved from Section 6.0 to Section 1 scope
#      exclusions (basisOfRecord = PreservedSpecimen; removed for inland
#      locality not taxonomic reasons).
#   2. 3 York Sound MAGNT records added to Section 1 scope exclusions
#      (stateProvince = Queensland but locality text consistent across 4
#      sources as York Sound, WA Kimberley coast).
# Section 1 now removes 13 records total (3 NSW/WA Fix2 + 10 scope_exclusions),
# result 49,446. Section 4 result 42,944. Final total 42,902.
# -----------------------------------------------------------------------------

pipeline_audit <- tribble(
  ~stage, ~records, ~notes,
  "Pipeline integration output", 67848, "12 sources merged, before catalogNumber case-insensitivity fix",
  "Section 6b fix applied (case-insensitive catalogNumber)", 49459, "36,232 case-mismatched duplicate rows collapsed",
  "Section 0: case-duplicate record_key removal", 49459, "0 case duplicates found (already resolved upstream)",
  "Section 1: coordinate fixes + geographic scope", 49446, "7 records removed: 3 NSW/WA outliers, 3 original scope exclusions (Darwin NT, inland fungus, Western Australia), PLUS 4 inland Disaster genus records (Yabbra State Forest / Mt Warning, NSW) - confirmed via direct source review that basisOfRecord = PreservedSpecimen, not FossilSpecimen as originally assumed, so removed here for inland locality, not taxonomic/fossil reasons",
  "Section 2: country fixes", 49446, "No records removed; country field standardised",
  "Section 3: year fixes", 49446, "No records removed; 4 impossible years + 40 historical placeholders corrected, plus best_year resolved via majority vote across year__* source columns (4 records with genuine cross-source disagreement, 0.01%)",  "Section 4: basis of record review", 42944, "6,502 removed: fossil/machine (45), living specimens (4,642), material samples (1,654), IMOS zooplankton (161)",
  "Section 5: depth resolution", 42944, "No records removed; best_min_depth/best_max_depth/depth_zone/depth_median resolved, including a post-audit fix correcting 3,411 records with reversed min/max depth values (genuine ALA_other/QM source data inconsistency, not a pipeline bug) plus 711 records recovered via event-sibling imputation (642) and reef flat/intertidal text inference (69)",
  "Section 6.0: non-echinoderm contaminant removal", 42902, "42 removed: Asterina eupomatiae fungus (1), Anoplura lice (2), Rhodostoma Mollusca genus (1, confirmed via WoRMS phylum check), ANWC mislabelled mammal specimens (38). Disaster genus is NOT removed here - reclassified to Section 1 geographic scope exclusions (see note there). All matched by exact record_key or exact institutionCode+collectionCode equality",
  "Section 6e: name normalisation + WoRMS resolution (conflicted names only)", 42902, "accepted_name/taxonomic_resolution_level assigned to 100% of records, but only a subset had accepted_name_worms TRUE (only conflicted names were queried)",
  "Section 6h: full WoRMS validation of every accepted_name", 42902, "Remaining never-conflicted names validated against WoRMS; names corrected to current WoRMS-valid combination (Hemiaster retained as-is, see Section 6.0 comments); 100% of records now have a confirmed WoRMS status via accepted_name_final",
  "Section 6i: post-audit fixes", 42902, "Two additional accepted_name_final corrections (ochroleucus grammatical agreement; intermedius superseded combination, and one alternative representation correction: Amphiodia (Amphispina) microplax, all confirmed via direct AphiaID lookup), plus is_extinct_genus flag added for Hemiaster",
  "Section 6j: echino_class resolution", 42902, "echino_class built from source class__* columns (case-insensitive, source-priority order); 144 records with no source class field but a finer-than-Phylum identification recovered via taxon (genus/family/order) -> class lookup, restricted to standard echinoderm taxonomy; 828 genuine Phylum-only records correctly left as NA; echino_class_source distinguishes original_source_field from inferred_from_taxon_name"
)
...
key_columns <- tribble(
  ...
  "is_historical_collection", "TRUE for MCZ records with placeholder collection years", "Section 3",
  ...
  "accepted_name_final", "FINAL authoritative name - current WoRMS-valid combination for ALL records, including the 27 corrected in 6h/6i. Use this column, not accepted_name, for analysis.", "Section 6h/6i",
  ...
  "taxonomic_resolution_level", "Rank derived from accepted_name_final word structure (recomputed in Section 6i to correct fallback_rank artefacts); values: Species/Genus/Family/Order/Class/Phylum/Subspecies/Subgenus", "Section 6e/6i",  "is_morphospecies", "TRUE if any source recorded a morphospecies designation (sp1, sp. A, etc.)", "Section 6b",
  ...
  "coord_land_qc_flag", "Distance-to-coast + coordinateUncertaintyInMeters based QC flag for georeferenced records that fall on land. not_on_land = no flag (includes both confirmed marine coordinates and the 227 records with no coordinates at all); on_land_near_coast_or_island = within 2km of coast, consistent with genuine small-island/reef-station localities (kept as-is); on_land_high_uncertainty = coordinateUncertaintyInMeters >= 10km; on_land_far_inland_review = >5km inland with no uncertainty flag; on_land_moderate_distance_review = ambiguous, needs manual review", "Section 1d",  "echino_class", "Resolved echinoderm class (Asteroidea/Ophiuroidea/Echinoidea/Holothuroidea/Crinoidea); NA only for genuine Phylum-only identifications", "Section 6j",
  "coord_qc_exclude_recommended", "TRUE only for on_land_high_uncertainty / on_land_far_inland_review (825 records, 1.92%). Convenience filter column - use this, not coord_land_qc_flag directly, to exclude low-confidence coordinates from an analysis", "Section 1d",
  ...
  "echino_class_source", "original_source_field if resolved directly from source class__* columns, inferred_from_taxon_name if recovered via taxon->class lookup, NA if still unresolved (Phylum-only)", "Section 6j"
)
